...
await describe("POST /api/admin/grants", async () => {
  ...
  await it("lets an admin grant Pulse by X handle", async () => {
    upsertOauthUser({
      provider: "x",
  ...
  await it("rejects a non-admin", async () => {
  ...
  await it("rejects a grant request with no Origin header", async () => {
    upsertOauthUser({
      provider: "x",
  ...
  await it("reports a stored-but-inert grant when a live Stripe sub wins", async () => {
    const target = upsertOauthUser({
      provider: "google",
  ...
  await it("does not claim Free when clearing a grant under a live Stripe sub", async () => {
    const target = upsertOauthUser({
      provider: "google",
  ...
  await it("reports the grant as live when the target's Stripe sub is incomplete", async () => {
    const target = upsertOauthUser({
      provider: "google",
  ...
  await it("reports a stored-but-inert grant for an admin-email target", async () => {
